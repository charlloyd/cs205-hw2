...
//Edited by Eric Dunipace to take new problem sizes
...
#define SIZE[4] = {pow(2,6),pow(2,10), pow(2,16)}

#define S = 4

__global__ void naive_mm(const int m, const int n, int k, const double *a, const double *b, double *c )
{
...
void cpu_mm(int N, int J, int K, double *x, double *y, double *z){
for( int n;)
}
